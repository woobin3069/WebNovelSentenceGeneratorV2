...
for file in os.listdir('./data'):
  if file != ".ipynb_checkpoints":
    all_files += unidecode.unidecode(open('./data/'+file, encoding='UTF8').read()) + "\n"
...
def random_training_set(chunk_len=5000):
...
if __name__ == "__main__":
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if epoch % print_every == 0:
            print('[%s taken (%d epochs %d%% trained) Loss: %.4f Perplexity: %.4f]' % (
            time_since(start), epoch, epoch / n_epochs * 100, loss, perplexity))
    ...
    # evaluation
    chunk = random_chunk(1000)
    prime_str, target_str = chunk[:502], chunk[502:]

    gen_text, loss, perplexity = evaluate(target_str, prime_str, 500, temperature=0.8)
    ...
    print(generate("\n맑은 기계음 소리가 들린 건 그때였다.\
    2019년 12월 31일 경기도 모 드라마 촬영장. ", 300, temperature=0.8))

    # Dracula - Bram Stoker
    print(generate("\n캡슐 안에 있던 냉기가 밖으로 빠져나가는 소리가 들렸다. ", 300, temperature=0.8))

    # outside evaluation

    # Emma - Jane Austen
    print(generate("\n실력이 부족해서, 혹은 애가 어디 모자라서 만년 대리로 있는 건 아니니까. ", 300, temperature=0.8))
